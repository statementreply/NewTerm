...
- (void)setForeground24BitColor:(UIColor *)color {
	CGFloat r, g, b;
	[color getRed:&r green:&g blue:&b alpha:nil];
	graphicRendition_.fgColorCode = r * 255.0;
	graphicRendition_.fgGreen = g * 255.0;
	graphicRendition_.fgBlue = b * 255.0;
...
- (UIColor *)colorForXtermCCSetPaletteString:(NSString *)argument colorNumberPtr:(int *)numberPtr {
	if ([argument length] == 7) {
		...
		if (count == 4 &&
			...
			b >= 0 &&
			b <= 255) {
			UIColor* srgb = [UIColor colorWithRed:((double)r)/255.0
...
- (NSString *)decodedBase64PasteCommand:(NSString *)commandString {
	...
	// TODO(kirb): this is using libapr, which seems unnecessary and is not included with iOS. port
	// this to built-in NSData APIs
	return nil;
	/*
...
- (void)executeToken:(VT100Token *)token {
	...
	switch (token->type) {
		...
		case XTERMCC_WINDOWPOS:
			[delegate_ terminalMoveWindowTopLeftPointTo:CGPointMake(token.csi->p[1], token.csi->p[2])];
			break;
		...
		case XTERMCC_REPORT_WIN_POS: {
			CGPoint topLeft = [delegate_ terminalWindowTopLeftPixelCoordinate];
		...
		if ((i % 2) == 0 ) {
			...
			if ([part hasPrefix:@"rgb:"]) {
				...
				if (components.count == 3) {
					...
					if (ok) {
						UIColor *theColor = [UIColor colorWithRed:colors[0]
																	green:colors[1]
																	 blue:colors[2]
																	alpha:1];
				...
				UIColor *theColor = [delegate_ terminalColorForIndex:theIndex];
...
- (void)executeXtermSetKvp:(VT100Token *)token {
	...
	if ([key isEqualToString:@"CursorShape"]) {
		...
		NSLog(@"Deprecated and unsupported code BeginFile received. Use File instead.");
	} else if ([key isEqualToString:@"EndFile"]) {
		NSLog(@"Deprecated and unsupported code EndFile received. Use File instead.");
...
- (void)executeXtermSetPalette:(VT100Token *)token {
	int n;
	UIColor *theColor = [self colorForXtermCCSetPaletteString:token.string
											   colorNumberPtr:&n];
	if (theColor) {
		switch (n) {
			case 16:
