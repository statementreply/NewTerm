...
- (UIFont *)fontValue {
    ...
    if ([self length] == 0) {
        return ([UIFont fallbackFixedPitchFontOfSize:0.0]);
    }
    ...
    aFont = [UIFont fontWithName:[NSString stringWithFormat:@"%s", utf8FontName] size:fontSize];
    if (aFont == nil) {
        return ([UIFont fallbackFixedPitchFontOfSize:0.0]);
    }
...
- (NSString *)_convertBetweenUTF8AndHFSPlusAsPrecomposition:(BOOL)precompose {
    // TODO(kirb): this used Carbon APIs. should we just not support this at all?
    return [self copy];
}
...
+ (NSString *)stringForModifiersWithMask:(NSUInteger)keyMods {
    // TODO(kirb): is this needed at all?
    return @"";
    /*
...
- (NSDictionary *)attributesUsingFont:(UIFont *)font fittingSize:(NSSize)maxSize attributes:(NSDictionary *)baseAttributes {
    ...
    while (points != prevPoints) {
        attributes[NSFontAttributeName] = [UIFont fontWithName:font.fontName size:points];
        NSRect boundingRect = [self boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX)
