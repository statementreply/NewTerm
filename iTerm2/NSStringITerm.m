...
+ (NSString *)stringFromPasteboard {
	// kirb: iOS doesn't specifically support file paths in the first place, so support for properly escaping those is removed
...
- (UIFont *)fontValue {
    ...
    if ([self length] == 0) {
        return ([UIFont fontWithName:@"Menlo-Regular" size:14.f]);
    }
...
static TECObjectRef CreateTECConverterForUTF8Variants(TextEncodingVariant variant) {
    ...
    if (TECCreateConverter(&conv, utf16Encoding, hfsPlusEncoding) != noErr) {
        HBLogWarn(@"Failed to create HFS Plus converter.\n");
        return NULL;
    }
...
- (NSString *)_convertBetweenUTF8AndHFSPlusAsPrecomposition:(BOOL)precompose {
	// TODO: kirb: this function needs to be either implemented or removed for iOS
	return self;
...
/*
// kirb: removed; not really needed on iOS
