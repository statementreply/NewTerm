...
void SetDecodedImage(unichar code, UIImage *image, NSData *data) {
...
int AppendToComplexChar(int key, unichar codePoint)
{
    ...
    if ([str length] == kMaxParts) {
        HBLogWarn(@"Warning: char <<%@>> with key %d reached max length %d", str,
              key, kMaxParts);
        return key;
    }
    assert(str);
...
void DumpScreenCharArray(screen_char_t* screenChars, int lineLength) {
    HBLogDebug(@"%@", ScreenCharArrayToStringDebug(screenChars, lineLength));
}
