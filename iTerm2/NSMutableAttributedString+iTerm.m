...
- (void)iterm_appendString:(NSString *)string {
    ...
    if (self.length > 0) {
        ...
        HBLogWarn(@"WARNING: iterm_appendString: to empty object %@ will have no attributes", self);
    }
...
- (CGFloat)heightForWidth:(CGFloat)maxWidth {
    ...
    CGSize size = CGSizeMake(maxWidth, FLT_MAX);
    NSTextContainer *textContainer =
        [[[NSTextContainer alloc] initWithSize:size] autorelease];
