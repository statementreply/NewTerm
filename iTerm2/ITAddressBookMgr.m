...
- (instancetype)init {
    ...
    if (self) {
        ...
        if ([newBookmarks isKindOfClass:[NSArray class]]) {
            ...
            HBLogDebug(@"Loading profiles from %@", newBookmarks);
            ...
            if ([self loadDynamicProfilesFromFile:(NSString *)newBookmarks
                ...
                HBLogError(@"Failed to load profiles from %@", newBookmarks);
                exit(1);
            }
        }
...
+ (NSDictionary*)encodeColor:(UIColor*)origColor {
...
+ (UIColor *)decodeColor:(NSDictionary*)plist {
...
{
    if ([self verbose]) {
        HBLogDebug(@"netServiceBrowser:%@ didFindService:%@ moreComing:%d",
...
- (void)netServiceBrowser:(NSNetServiceBrowser *)aNetServiceBrowser
    ...
    if ([self verbose]) {
        HBLogDebug(@"netServiceBrowser:%@ didRemoveService:%@ moreComing:%d",
              aNetServiceBrowser, aNetService, (int)moreComing);
    }
    ...
    for (NSNumber* n in [[ProfileModel sharedInstance] bookmarkIndicesMatchingFilter:@"tag:^bonjour$"]) {
        ...
        if ([bookmarkName isEqualToString:[aNetService name]]
            ...
            if ([self verbose]) {
                HBLogDebug(@"remove profile with name %@", bookmarkName);
            }
...
- (void)netServiceDidResolveAddress:(NSNetService *)sender
{
    ...
    if ([self verbose]) {
        HBLogDebug(@"netServiceDidResolveAddress:%@", sender);
    }
    if ([bonjourServices containsObject: sender] == NO) {
        if ([self verbose]) {
            HBLogDebug(@"netServiceDidResolveAddress sender not in services %@", bonjourServices);
        }
        return;
    }
    ...
    if ([[sender addresses] count] == 0) {
        if ([self verbose]) {
            HBLogDebug(@"netServiceDidResolveAddress sender has no addresses");
        }
        return;
    }
    ...
    if ([self verbose]) {
        HBLogDebug(@"netServiceDidResolveAddress type=%@ name=%@ address=%@", serviceType, serviceName, address);
    }
    ...
    if (strAddr) {
        if ([self verbose]) {
            HBLogDebug(@"netServiceDidResolveAddress add profile with address %s", strAddr);
        }
...
{
    if ([self verbose]) {
        HBLogDebug(@"getBonjourServiceType:%@", aType);
    }
    NSString *serviceType = aType;
    if ([aType length] <= 0) {
        if ([self verbose]) {
            HBLogDebug(@"netServiceDidResolveAddress empty type");
        }
        return nil;
    }
    ...
    if(aRange.location != NSNotFound) {
        if ([self verbose]) {
            HBLogDebug(@"netServiceDidResolveAddress return value prior to first .");
        }
        return [serviceType substringWithRange: NSMakeRange(1, aRange.location - 1)];
    } else {
        if ([self verbose]) {
            HBLogDebug(@"netServiceDidResolveAddress no . found, return whole value");
        }
        return serviceType;
    }
}
    ...
    if ([[bookmark objectForKey:KEY_CUSTOM_DIRECTORY] isEqualToString:kProfilePreferenceInitialDirectoryAdvancedValue]) {
        switch (objectType) {
            ...
            case iTermPaneObject:
                ...
                break;
            default:
                HBLogDebug(@"Bogus object type %d", (int)objectType);
...
- (void)reloadDynamicProfiles {
    ...
    for (NSString *file in fileNames) {
        ...
        if (![self loadDynamicProfilesFromFile:fullName intoArray:newProfiles guids:guids]) {
            HBLogDebug(@"Igoring dynamic profiles in malformed file %@ and continuing.", fullName);
        }
    }
...
- (BOOL)loadDynamicProfilesFromFile:(NSString *)filename
    ...
    if (!dict) {
        ...
        if (!data) {
            HBLogDebug(@"Dynamic Profiles file %@ is unreadable", filename);
            return NO;
        }
        NSError *error = nil;
        dict = [NSJSONSerialization JSONObjectWithData:data
                                               options:0
                                                 error:&error];
        if (!dict) {
            HBLogDebug(@"Dynamic Profiles file %@ doesn't contain a valid property list", filename);
            return NO;
        }
    }
    NSArray *entries = dict[@"Profiles"];
    if (!entries) {
        HBLogDebug(@"Property list in %@ has no entries", entries);
        return NO;
    }
    for (Profile *profile in entries) {
        if (![profile[KEY_GUID] isKindOfClass:[NSString class]]) {
            HBLogDebug(@"Dynamic profile is missing the Guid field in file %@", filename);
            continue;
        }
        if (![profile[KEY_NAME] isKindOfClass:[NSString class]]) {
            HBLogDebug(@"Dynamic profile with guid %@ is missing the name field", profile[KEY_GUID]);
            continue;
        }
        if ([self nonDynamicProfileHasGuid:profile[KEY_GUID]]) {
            HBLogDebug(@"Dynamic profile with guid %@ conflicts with non-dynamic profile with same guid", profile[KEY_GUID]);
            continue;
        }
        if ([guids containsObject:profile[KEY_GUID]]) {
            HBLogDebug(@"Two dynamic profiles have the same guid: %@", profile[KEY_GUID]);
            continue;
        }
...
- (Profile *)prototypeForDynamicProfile:(Profile *)profile {
    ...
    if (parentName) {
        ...
        if (!prototype) {
            HBLogDebug(@"Dynamic profile %@ references unknown parent name %@. Using default profile as parent.",
...
- (void)removeDynamicProfile:(Profile *)profile {
    [[iTermPreferences sharedInstance] removeProfileWithGuid:profile[KEY_GUID]];
}
