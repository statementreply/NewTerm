...
{
    HBLogDebug(@"%@", [self compactLineDumpWithWidth:width andContinuationMarks:NO]);
}
           ...
           continuation:(screen_char_t *)continuationPtr
{
    ...
    HBLogError(@"Couldn't find line %d", lineNum);
...
{
    int i;
    int position = (int)start.absolutePosition - (int)droppedChars;
...
- (NSArray*)convertPositions:(NSArray*)resultRanges withWidth:(int)width
{
    ...
    for (NSNumber* positionNum in positionsArray) {
        ...
        if (i < numBlocks) {
            ...
            if (isOk) {
                y += yoffset;
                [intermediate setObject:[NSValue valueWithCGPoint:CGPointMake(x, y)]
                                 forKey:positionNum];
            } else {
                assert(false);
            }
    ...
    for (ResultRange* rr in resultRanges) {
        ...
        if (start && end) {
            ...
            CGPoint startPoint = [start CGPointValue];
            CGPoint endPoint = [end CGPointValue];
                                     ...
                                     ok:(BOOL *)ok
{
    ...
    int i;
    int yoffset = 0;
    int p = (int)position.absolutePosition - (int)droppedChars;
